...
class Bicross_att(nn.Module):
    ...
    def forward(self, x, y, ret_attn_mask=False):
        ...
        # print(x.shape)
        # patchify, (n, p^2, w, w, c), keep 2d window as we need 2d pooling to reduce kv size
        # nn_win = int(self.n_win / 2)
        nn_win = self.n_win

        # print(type(nn_win))
        x = rearrange(x, "n (j h) (i w) c -> n (j i) h w c", j=self.n_win, i=self.n_win)
        y = rearrange(y, "n (j h) (i w) c -> n (j i) h w c", j=nn_win, i=nn_win)
        # y = rearrange(y, "n (j h) (i w) c -> n (j i) h w c", j=2, i=2)
        ...
        ############ y #################
        # print('yyyyyyyyyyy')
        q_pixy = rearrange(qy, 'n p2 h w c -> n p2 (h w) c')
        kv_pixy = self.kv_down(rearrange(kvy, 'n p2 h w c -> (n p2) c h w'))
        kv_pixy = rearrange(kv_pixy, '(n j i) c h w -> n (j i) (h w) c', j=self.n_win, i=self.n_win)
        # print(kv_pixy.shape)
        q_winy, k_winy = qy.mean([2, 3]), kvy[..., 0:self.qk_dim].mean([2, 3]) # window-wise qk, (n, p^2, c_qk), (n, p^2, c_qk)
        # print(q_winy.shape)
        # print(k_winy.shape)
        ##################side_dwconv(lepe)##################
        # NOTE: call contiguous to avoid gradient warning when using ddp
        lepey = self.lepe(rearrange(kvy[..., self.qk_dim:], 'n (j i) h w c -> n c (j h) (i w)', j=nn_win, i=nn_win).contiguous())
        lepey = rearrange(lepey, 'n c (j h) (i w) -> n (j h) (i w) c', j=nn_win, i=nn_win)
        # print(lepey.shape)
        ...
        attn_weight = (q_pix * self.scale) @ k_pix_sel # (n*p^2, m, w^2, c) @ (n*p^2, m, c, topk*h_kv*w_kv) -> (n*p^2, m, w^2, topk*h_kv*w_kv)
        attn_weight = self.attn_act(attn_weight)
        out1 = attn_weight @ v_pix_sel # (n*p^2, m, w^2, topk*h_kv*w_kv) @ (n*p^2, m, topk*h_kv*w_kv, c) -> (n*p^2, m, w^2, c)
        out1 = rearrange(out1, '(n j i) m (h w) c -> n (j h) (i w) (m c)', j=self.n_win, i=self.n_win,
                        h=H//self.n_win, w=W//self.n_win)
        
        # kv_pix = kv_pix + kv_pixy
        kv_pix = kv_pix + kv_pixy
        

        ###############crossxy####################################
        r_weight, r_idx = self.router(q_win, k_winy) # both are (n, p^2, topk) tensors
        kv_pix_sel = self.kv_gather(r_idx=r_idx, r_weight=r_weight, kv=kv_pix) #(n, p^2, topk, h_kv*w_kv, c_qk+c_v)
        _, v_pix_sel = kv_pix_sel.split([self.qk_dim, self.dim], dim=-1)
        kv_pix_sel = self.kv_gather(r_idx=r_idx, r_weight=r_weight, kv=kv_pixy)
        k_pix_sel, _ = kv_pix_sel.split([self.qk_dim, self.dim], dim=-1)
        ######### do attention as normal ####################
        k_pix_sel = rearrange(k_pix_sel, 'n p2 k w2 (m c) -> (n p2) m c (k w2)', m=self.num_heads) # flatten to BMLC, (n*p^2, m, topk*h_kv*w_kv, c_kq//m) transpose here?
        v_pix_sel = rearrange(v_pix_sel, 'n p2 k w2 (m c) -> (n p2) m (k w2) c', m=self.num_heads) # flatten to BMLC, (n*p^2, m, topk*h_kv*w_kv, c_v//m)
        # q_pix = rearrange(q_pix, 'n p2 w2 (m c) -> (n p2) m w2 c', m=self.num_heads) # to BMLC tensor (n*p^2, m, w^2, c_qk//m)
...
class Bicross_att_cat(nn.Module):
    ...
    def forward(self, x, y, ret_attn_mask=False):
        ...
        r_weight, r_idx = self.router(q_win, k_winy) # both are (n, p^2, topk) tensors
        kv_pix_sel = self.kv_gather(r_idx=r_idx, r_weight=r_weight, kv=kv_pix) #(n, p^2, topk, h_kv*w_kv, c_qk+c_v)
        kv_pix_sely = self.kv_gather(r_idx=r_idx, r_weight=r_weight, kv=kv_pixy) #(n, p^2, topk, h_kv*w_kv, c_qk+c_v)
        k_pix_sel, v_pix_sely = kv_pix_sely.split([self.qk_dim, self.dim], dim=-1)
        _, v_pix_sel = kv_pix_sel.split([self.qk_dim, self.dim], dim=-1)
        ######### do attention as normal ####################
        k_pix_sel = rearrange(k_pix_sel, 'n p2 k w2 (m c) -> (n p2) m c (k w2)', m=self.num_heads) # flatten to BMLC, (n*p^2, m, topk*h_kv*w_kv, c_kq//m) transpose here?
        v_pix_sel = rearrange(v_pix_sel, 'n p2 k w2 (m c) -> (n p2) m (k w2) c', m=self.num_heads) # flatten to BMLC, (n*p^2, m, topk*h_kv*w_kv, c_v//m)
        v_pix_sely = rearrange(v_pix_sely, 'n p2 k w2 (m c) -> (n p2) m (k w2) c', m=self.num_heads) # flatten to BMLC, (n*p^2, m, topk*h_kv*w_kv, c_v//m)
        v_pix_sel = torch.cat([v_pix_sel,v_pix_sely], dim=3)
        # q_pix = rearrange(q_pix, 'n p2 w2 (m c) -> (n p2) m w2 c', m=self.num_heads) # to BMLC tensor (n*p^2, m, w^2, c_qk//m)
        # param-free multihead attention
        attn_weight = (q_pix * self.scale) @ k_pix_sel # (n*p^2, m, w^2, c) @ (n*p^2, m, c, topk*h_kv*w_kv) -> (n*p^2, m, w^2, topk*h_kv*w_kv)
        attn_weight = self.attn_act(attn_weight)
        ...
        out = rearrange(out, '(n j i) m (h w) c -> n (j h) (i w) (m c)', j=self.n_win, i=self.n_win,
                        h=H//self.n_win, w=W//self.n_win)
        # lepe = torch.cat([lepe, lepey],dim=3)
        # print(out.shape)
        # print(lepey.shape)
        # print(out1.shape)

        out = torch.cat([out, lepey],dim=3)
        # print(out.shape)
        # output linear
        out = self.wo(out)
        # print(out.shape)
...
class Block_cross(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=-1,
                       ...
                       side_dwconv=5, before_attn_dwconv=3, pre_norm=True, auto_pad=False, if_cat=False):
        super().__init__()
        ...
        if before_attn_dwconv > 0:
            ...
            # self.pos_embed_y = nn.Conv2d(dim, 2*dim,  kernel_size=before_attn_dwconv, padding=1, groups=dim)
        else:
            self.pos_embed = lambda x: 0
        self.norm1 = nn.LayerNorm(dim, eps=1e-6) # important to avoid attention collapsing
        if topk > 0:
            if if_cat == False:
                self.attn = Bicross_att(dim=dim, num_heads=num_heads, n_win=n_win, qk_dim=qk_dim,
                ...
                self.attn = Bicross_att_cat(dim=dim, num_heads=num_heads, n_win=n_win, qk_dim=qk_dim,
                                            qk_scale=qk_scale, kv_per_win=kv_per_win, kv_downsample_ratio=kv_downsample_ratio,
                                            kv_downsample_kernel=kv_downsample_kernel, kv_downsample_mode=kv_downsample_mode,
                                            topk=topk, param_attention=param_attention, param_routing=param_routing,
                                            diff_routing=diff_routing, soft_routing=soft_routing, side_dwconv=side_dwconv,
    ...
    def forward(self, z):
        ...
        if self.pre_norm:
            if self.use_layer_scale:
                x = x + self.drop_path(self.gamma1 * self.attn(self.norm1(x),self.norm1(y))) # (N, H, W, C)
                # print('2')
                x = x + self.drop_path(self.gamma2 * self.mlp(self.norm2(x))) # (N, H, W, C)
                # print('1')
            else:
                x = x + self.drop_path(self.attn(self.norm1(x),self.norm1(y))) # (N, H, W, C)
...
class BiFormer_cross(nn.Module):
    def __init__(self, embed_dim=[64, 64], depth=[1], in_chans=64, num_classes=1000, 
                 head_dim=4, qk_scale=None, representation_size=None,
        ...
        # print(dp_rates)
        # dp_rates=[x for x in torch.linspace(0, drop_path_rate, sum(depth))] 
        # dp_rates = [0.0]
        # print(depth)
        cur = 0
        self.l = len(depth)
        for i in range(len(depth)):
            stage = nn.Sequential(
                *[Block_cross(dim=embed_dim[i], drop_path=dp_rates[cur + j], 
                        ...
                        pre_norm=pre_norm,
                        auto_pad=auto_pad,
                        if_cat=if_cat) for j in range(depth[i])],
            )
        ...
        if self.l == 1:
            self.d = nn.Sequential(
                nn.Conv2d(embed_dim[0], embed_dim[1], kernel_size=(1, 1)),
                nn.BatchNorm2d(embed_dim[1])
            )
            # self.up = nn.Upsample(scale_factor=4.0)
        ##########################################################################
    ...
    def forward(self, x):
        # print(x.shape)
        x = [x[:,0:self.embed_dim[0],:,:], x[:,self.embed_dim[0]:,:,:]]
...
class BiFormer(nn.Module):
    def __init__(self, depth=[1], in_chans=3, num_classes=1000, embed_dim=[128, 64],
                 head_dim=2, qk_scale=None, representation_size=None,
                 drop_path_rate=0., drop_rate=0.,
                 use_checkpoint_stages=[],
                 ######## 
                 n_win=4,
        ...
        #         nn.Conv2d(embed_dim[i], embed_dim[i], kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)),
