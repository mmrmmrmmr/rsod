...
class add_w(nn.Module):
    ...
    def __init__(self, dimension=1):
        ...
        self.c1_1 = nn.Sequential(
                    Conv(self.d, 2*self.d),
                    Conv(2*self.d, self.d),
                    nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        ci1 = self.c1_1(i1)
        ci2 = self.c2_2(i2)
        # cm1 = ci1.mean([1, 3]).unsqueeze(dim=1).unsqueeze(dim=3) 
        # cm2 = ci2.mean([1, 2]).unsqueeze(dim=1).unsqueeze(dim=2) 
        cm1 = ci1.mean([1, 3]).unsqueeze(dim=1).unsqueeze(dim=3) 
        cm2 = ci2.mean([1, 2]).unsqueeze(dim=1).unsqueeze(dim=2) 
        cc1 = torch.mul(cm1, cm2)
        cc1 = self.s(cc1)
        cm1 = ci2.mean([1, 3]).unsqueeze(dim=1).unsqueeze(dim=3) 
        cm2 = ci1.mean([1, 2]).unsqueeze(dim=1).unsqueeze(dim=2) 
        cc2 = torch.mul(cm1, cm2)
        cc2 = self.s(cc2)
        # r_weight, r_idx = self.t(cm1, cm2)
        # kv_pix_sel = self.g(r_idx=r_idx, r_weight=r_weight, kv=cm2)
        
        # return kv_pix_sel
        return i1*cc2 + i2*cc1


class add_w2(nn.Module):
    """Concatenates tensors along a specified dimension for efficient tensor manipulation in neural networks."""
    ...
    def forward(self, x):
        ...
        # cm1 = ci1.mean([1, 3]).unsqueeze(dim=1).unsqueeze(dim=3) 
        # cm2 = ci2.mean([1, 2]).unsqueeze(dim=1).unsqueeze(dim=2) 
        cm1 = ci1.mean([3]).unsqueeze(dim=3) 
        cm2 = ci2.mean([2]).unsqueeze(dim=2) 
        cc1 = torch.mul(cm1, cm2)
        cc1 = self.s(cc1)
        # cm1 = ci2.mean([3]).unsqueeze(dim=3) 
        # cm2 = ci1.mean([2]).unsqueeze(dim=2) 
        # cc2 = torch.mul(cm1, cm2)
        # cc2 = self.s(cc2)
        # r_weight, r_idx = self.t(cm1, cm2)
        # kv_pix_sel = self.g(r_idx=r_idx, r_weight=r_weight, kv=cm2)
        ...
        # return kv_pix_sel
        return self.c(torch.cat([i1*cc1 + i2, i1+i2], dim=1))

class add_w3(nn.Module):
    """Concatenates tensors along a specified dimension for efficient tensor manipulation in neural networks."""
