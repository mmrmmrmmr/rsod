...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
        ...
        self.lw = line_width or max(round(sum(im.shape) / 2 * 0.003), 0.5)  # line width
        if self.pil:  # use PIL
            self.im = im if isinstance(im, Image.Image) else Image.fromarray(im)
            self.draw = ImageDraw.Draw(self.im)
            try:
                font = check_font("Arial.Unicode.ttf" if non_ascii else font)
                size = font_size or max(round(sum(self.im.size) / 2 * 0.035), 8)
            ...
            self.tf = int(max(self.lw - 1, 0.1))  # font thickness
            self.sf = self.lw / 7  # font scale
        # Pose
        self.skeleton = [
            [16, 14],
            [14, 12],
