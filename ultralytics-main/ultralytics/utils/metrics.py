...
def bbox_iou_alpha(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7, a=1, ab=False, abc=False, gwh=False, gl=False, eiou=False, e=False, g22=False):
    """
    ...
    iou = torch.pow(iou, a)
    # print(iou.shape)
    if CIoU or DIoU or GIoU or True:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or ab or abc or eiou or e:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                return iou - (torch.pow(rho2 / c2, a) + torch.pow(v * alpha, a))  # CIoU
            if ab or abc:
                a2 = w1**2 + h1**2 + eps
                b2 = w2**2 + h2**2 + eps
                if ab:
                    # print('yyyyyyyyyyyyyyyyyyy')
                    bbb = torch.abs(a2-b2) / (a2+b2)
                    with torch.no_grad():
                        # alpha = v / (v - iou + (1 + eps))
                        bb =  bbb / (bbb - iou + (1 + eps))    
                    return  iou - torch.pow(rho2 / c2, a) - (torch.pow(bb*bbb, a))
                if abc:
                    ...
                    with torch.no_grad():
                        alpha = v / (v - iou + (1 + eps))
                        bb =  bbb / (bbb - v + (1 + eps))
                    return iou - (torch.pow(rho2 / c2, a) + torch.pow(v * alpha, a)) - (torch.pow(bb*bbb, a)) # CIoU
            if eiou:
                return iou - torch.pow(rho2 / c2, a) - torch.pow((w1-w2)**2/cw**2, a) - torch.pow((h1-h2)**2/ch**2, a)
            if e:
                ww = (w1-w2)**2/cw**2
                hh = (h1-h2)**2/ch**2
                t = rho2 / c2
                alpha = t / (t - iou + (1 + eps))
                return iou - torch.pow(t, a) - hh * alpha - ww * alpha
            return iou - rho2 / c2  # DIoU
        c_area = cw * ch + eps  # convex area
        f = (c_area - union) / c_area
        if GIoU:
            return iou - torch.pow(f, a)  # GIoU https://arxiv.org/pdf/1902.09630.pdf
        elif gwh:
            k = (w1+h1)/(w2+h2)
            r = (k-1)/(k+1)
            bb =  r / (r - iou + (1 + eps))
            return iou - torch.pow(f, a) - torch.pow(bb*r, a)
        elif gl:
            r = ((w1-w2)**2/(cw)+(h1-h2)**2/(ch))/(cw+ch)
            bb =  r / (r - iou + (1 + eps))
            return iou - torch.pow(f, a) - torch.pow(bb*r, a)
        elif g22:
            c2 = cw**2 + ch**2 + eps  # convex diagonal squared
            rho2 = ((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 + (b2_y1 + b2_y2 - b1_y1 - b1_y2) ** 2) / 4  # center dist ** 2
            return iou - torch.pow(f, a) - torch.pow(rho2 / c2, a) - (w1-w2)**2/cw**2 - (h1-h2)**2/ch**2
    return iou  # IoU
