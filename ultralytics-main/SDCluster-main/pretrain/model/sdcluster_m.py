...
# import sys
# sys.path.append('/home/mamingrui/sod/ultralytics-main/')
# from train_1_pre import backbone1, backbone2
# print('xxxxxxxxxxxxxxx')
...
class SDCLuster(nn.Module):
    def __init__(self, args, model_path_yaml):
        super().__init__()
        self.args = args
        ...
        self.encoder_q, self.encoder_k = get_b(model_path_yaml)
    ...
    def forward(self, input):
        ...
        for i in range(len(crops)):
            feature_list_q.append(self.encoder_q(crops[i]))
            with torch.no_grad():  # no gradient to keys[-1][-1]
                feature_list_k.append(self.encoder_k(crops[i]))
...
if __name__ == '__main__':
    ...
    args['train']['num_instances'] = 6 * 64
    args['train']['batch_size'] = 64
    args['train']['start_epoch'] = 1
    model = SDCLuster(args=args, model_path_yaml='/home/mamingrui/sod/ultralytics-main/my_model/vhr_yolov8s_pre_res.yaml')
    x =torch.randn([1,3,640,640])
    xx = model.encoder_k(x)
    xxp = model.projector_q(xx)