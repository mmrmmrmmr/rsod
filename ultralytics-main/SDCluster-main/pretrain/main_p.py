...
class Trainer():
    def __init__(self, args, rank, path, model_path_yaml):
        self.args = args
        print(self.args)
        self.rank = rank
        self.path = path
        # model
        self.net = SDCLuster(args=args, model_path_yaml=model_path_yaml).to(rank)
    ...
    def save(self):
        if self.rank == 0:
            # torch.save(self.net.module.encoder_k.state_dict(), self.path + '/backbone1.pt')
            torch.save(self.net.encoder_k.state_dict(), self.path + '/backbone1.pt')
    ...
    def sample(self, views, iter):
        ...
        for i in range(len(views)):
            if self.rank == 0:
                vutils.save_image(views[i] * stdimg + meanimg, self.path + '/{str(iter)}_img_{str(i)}.png',
                                  normalize=True)
# [-1]
            _, probs = self.net.grouping_k(
                self.net.projector_k(self.net.encoder_k(views[i])))
            ...
            if self.rank == 0:
                cv2.imwrite(self.path + '/{str(iter)}_out_cluster_{str(i)}.png', heatmap)
...
def main(rank, world_size, args, dataset, path='', model_path_yaml=None):
    print('rank: ', rank)
    ...
    trainer = Trainer(args, rank, path, model_path_yaml)
...
def all_pre(path, data_path, model_path_yaml):
    args = get_config('/home/mamingrui/sod/ultralytics-main/SDCluster-main/pretrain/config/config_pretrain.yaml')
    # args = get_config('config/config_pretrain.yaml')

    world_size = torch.cuda.device_count()
    print('world_size: ', world_size)

    dataset = DataSet_iSAID_Pretrain(args, data_path)
    main(0, world_size, args, dataset, path, model_path_yaml)
    # mp.spawn(main, args=(world_size, args, dataset, path, model_path_yaml), nprocs=world_size)     

if __name__ == '__main__':
    all_pre('/home/mamingrui/sod/ultralytics-main/SDCluster-main/save', '/home/mamingrui/sod/VHR/images/', '/home/mamingrui/sod/ultralytics-main/my_model/vhr_yolov8s_pre_res.yaml')
